...
fn qc(config: &mut Vec<Consist>, datetime: &NaiveDateTime, data: f64) -> bool {
    ...
    for conf in config {
        ...
        if conf.upper.len()==0 && conf.lower.len()==0 {
        ...
        while data > udata || (*datetime - utime).num_seconds() > conf.interval_to_sec() as i64 {
        ...
        while data < ldata || (*datetime - ltime).num_seconds() > conf.interval_to_sec() as i64 {
